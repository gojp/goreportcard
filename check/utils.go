...
func getFileSummaryMap(out *bufio.Scanner, dir, fnPrefix string) (map[string]FileSummary, error) {
	fsMap := make(map[string]FileSummary)
outer:
	for out.Scan() {
		filename := strings.Split(out.Text(), ":")[0]

		filename = strings.TrimPrefix(filename, fnPrefix)
		for _, skip := range skipSuffixes {
		...
		if autoGenerated(fnPrefix + filename) {
			continue outer
		}
...
func GoTool(dir string, filenames, command []string) (float64, []FileSummary, error) {
	...
	fsMap, err := getFileSummaryMap(out, dir, fnPrefix)
	if err != nil {
		return 0, []FileSummary{}, err
	}
