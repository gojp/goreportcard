...
func fileURL(dir, filename string) string {
	var fileURL string
	sep := "/" // here we are on "path" level
	dir = filepath.ToSlash(dir)
	base := strings.TrimPrefix(dir, filepath.ToSlash(repoSrc)+sep)
	switch {
	case strings.HasPrefix(base, "golang.org/x/"):
		var pkg string
		if len(strings.Split(base, sep)) >= 3 {
			pkg = strings.Split(base, sep)[2]
		}
		return fmt.Sprintf("https://github.com/golang/%s/blob/master%s", pkg, strings.TrimPrefix(filename, sep+base))
	case strings.HasPrefix(base, "github.com/"):
		if len(strings.Split(base, sep)) == 4 {
			base = strings.Join(strings.Split(base, sep)[0:3], sep)
		}
		return fmt.Sprintf("https://%s/blob/master%s", base, strings.TrimPrefix(filename, sep+base))
	case strings.HasPrefix(base, "gopkg.in/"):
		fmt.Println(goPkgInToGitHub(base))
		fmt.Println(strings.TrimPrefix(filename, sep+base))
		return goPkgInToGitHub(base) + strings.TrimPrefix(filename, sep+base)
	}
...
func makeFilename(fn string) string {
	...
	switch {
	case strings.HasPrefix(fn, sep+"github.com"):
		if len(sp) > 3 {
			return filepath.Join(sp[3:]...)
		}
	case strings.HasPrefix(fn, sep+filepath.Join("golang.org", "x")):
		if len(sp) > 3 {
			return filepath.Join(sp[3:]...)
		}
	case strings.HasPrefix(fn, sep+"gopkg.in"):
		if len(sp) > 3 {
			return filepath.Join(sp[3:]...)
		}
	}
...
outer:
	for out.Scan() {
		...
		filename = strings.TrimPrefix(filename, repoSrc)
		for _, skip := range skipSuffixes {
		...
		if autoGenerated(filepath.Join(repoSrc, filename)) {
			continue outer
		}
		...
		if fs.Filename == "" {
			...
			fs.FileURL = fileURL(dir, filename)
		}
...
func GoTool(dir string, filenames, command []string) (float64, []FileSummary, error) {
	...
	params = append(params, filepath.Join(dir, "..."))
	...
	go func(stopChan chan bool) {
		for _, f := range filenames {
			...
			go func(c chan FileSummary, errChan chan error, f string) {
				...
				if !bytes.Equal(b, g) {
					filename := strings.TrimPrefix(f, repoSrc)
					fs := FileSummary{}
					fs.Filename = makeFilename(filename)
					fs.FileURL = fileURL(dir, filename)
